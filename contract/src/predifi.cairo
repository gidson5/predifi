    ...
    impl predifi of IPredifi<ContractState> {
        ...
        fn vote_in_pool(
            ...
            if option == 1 {
                ...
                shares = self.calculate_user_shares(amount, pool.totalStakeOption1, pool.totalSharesOption1, pool.initial_share_price);
                pool.totalSharesOption1 += shares;
            } else {
                pool.totalStakeOption2 += amount;
                shares = self.calculate_user_shares(amount, pool.totalStakeOption2, pool.totalSharesOption2, pool.initial_share_price);
...
fn update_user_stake(ref self: ContractState, user: ContractAddress, amount: u256, shares: u256, option: u8) {
...
fn calculate_user_shares(ref self: ContractState,user_stake: u256,total_stake: u256,total_shares: u256, odds: u16) -> u256 {
